        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            {orders.map((order) => {
              ...
              const orderId = (order._id || order.id || '').toString();
              const orderNumber = order.order_number || orderId.slice(-8).toUpperCase();
              ...
              return (
                <Card 
                  key={orderId} 
                  className="border border-border rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden"
                >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>{createdDate.toLocaleDateString(locale, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <Clock className="h-3.5 w-3.5" />
                            <span>{createdDate.toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' })}</span>
                          </div>
                          <div className="hidden sm:flex items-center gap-1.5">
                            <Package className="h-3.5 w-3.5" />
                            <span>{getTranslation('orderType', order.order_type, language)}</span>
                          </div>
                        </div>
                      </div>
                      
                      {/* Status Badge */}
                      <div className={`flex items-center gap-2 px-4 py-2 rounded-full border ${statusConf.borderColor} ${statusConf.bgColor}`}>
                        <StatusIcon className={`h-4 w-4 ${statusConf.color}`} strokeWidth={2.5} />
                  ...
                  <CardContent className="space-y-6">
                    ...
                    {order.status !== 'CANCELLED' && order.status !== 'DELIVERED' && (
                      <div className="bg-muted/30 rounded-xl p-4">
                        <h4 className="text-sm font-semibold text-foreground mb-3">Order Progress</h4>
                        <div className="flex items-center justify-between relative">
                          {/* Progress Line */}
                          <div className="absolute top-5 left-0 right-0 h-0.5 bg-border" style={{ width: 'calc(100% - 40px)', marginLeft: '20px' }}></div>
                          <div 
                            className="absolute top-5 left-0 h-0.5 bg-primary transition-all duration-500" 
                            style={{ 
                              width: `calc(${(timeline.findIndex(s => s.active) / (timeline.length - 1)) * 100}% - ${40 * (timeline.findIndex(s => s.active) / (timeline.length - 1))}px)`,
                              marginLeft: '20px'
                            }}
                          ></div>
                            ...
                            return (
                              <div key={step.key} className="flex flex-col items-center gap-2 relative z-10">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
                                  ...
                                  <StepIcon className="h-4 w-4" strokeWidth={2.5} />
                                </div>
                              ...
                              <div className="flex justify-between items-start mb-2">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="font-semibold text-foreground">{item.meal_name || 'Meal'}</span>
                                  </div>
                                  
                                  {/* Base meal price */}
                                  {typeof item.meal_price === 'number' && (
                                    <div className="text-xs text-muted-foreground ml-8">
                                      {formatPrice(item.meal_price, language)} each
                                    </div>
                                  )}

                                  {/* Added ingredients */}
                                  {item.selected_ingredients && item.selected_ingredients.length > 0 && (
                                    <div className="text-xs text-emerald-600 dark:text-emerald-400 ml-8 mt-1">
                                          ...
                                          {item.selected_ingredients.map((ing: any, ingIdx: number) => {
                                            ...
                                            return (
                                              <span key={ingIdx} className="inline-flex items-baseline">
                                                ...
                                                {pr && <span className="text-[10px] ml-0.5">{pr}</span>}
                                  ...
                                  {item.removed_ingredients && item.removed_ingredients.length > 0 && (
                                    <div className="text-xs text-red-600 dark:text-red-400 ml-8 mt-1 flex items-start gap-1">
                                      <span className="font-semibold">−</span>
                                      <span>{(item.removed_ingredients_names && item.removed_ingredients_names.length > 0
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-2">
                      ...
                      {(order.status === 'PENDING' || order.status === 'CONFIRMED') && (
                        <Button
                          ...
                          className="rounded-xl text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 border-red-200 dark:border-red-900 hover:border-red-300 dark:hover:border-red-800 transition-colors"
                        >
                      ...
                      {order.status !== 'CANCELLED' && (
                        <Button
                          ...
                          className="rounded-xl hover:bg-primary/5 hover:text-primary hover:border-primary/50 transition-colors"
                        >
                          {isReordering ? (
                            <>
